...
from .utils.helper import dct2device, is_video, is_template, remove_suffix, is_image, is_square_video, \
...
class LivePortraitAnnotationPipeline(object):
    ...
    def execute(self, args: ArgumentConfig):
        ...
        assert os.path.exists(input_video_dir)
        for root, dirs, files in os.walk(input_video_dir):
            for file in files:
                if file.lower().endswith(('.mp4', '.avi', '.mov')):
                    curr_driving_video_path = Path(root) / file
                    relative_path = Path(root).relative_to(input_video_dir)
                    output_annotation_file_path = Path(output_annotation_dir) / relative_path / (
                            file.split('.')[0] + '.pth')

                    args.driving = str(curr_driving_video_path)
                    args.output_annotation_file = str(output_annotation_file_path)
                    os.makedirs(output_annotation_file_path.parent, exist_ok=True)

                    self.execute_video(args)

    def execute_video(self, args: ArgumentConfig):
        x_d_map = {}
        ...
        source_rgb_lst = load_video(args.driving)
        ...
        for i in track(range(n_frames), description='🚀Animating...', total=n_frames):
            if flag_is_source_video:  # source video
                ...
                if flag_source_video_eye_retargeting and source_lmk is not None:
                    ...
                    eye_delta_before_animation = self.live_portrait_wrapper.retarget_eye(x_s,
